...
const useDataStore = create(
  persist(
    (set, get) => ({
      ...
      setIndentCount: (count) => set({ indentCount: count }),
      setIndentCountsByStatus: (counts) => set({ indentCountsByStatus: counts }),
      updateIndentCounts: (data) => {
        if (!Array.isArray(data)) return;
        
        const counts = {
          total: data.length,
          pending: data.filter(item => {
            // Check if item has status or NeedMore at index 11
            const status = item.status || item[11] || '';
            return status === 'NeedMore' || status === 'Pending' || status === '';
          }).length,
          approved: data.filter(item => {
            const status = item.status || item[11] || '';
        ...
        set({ 
          indentCount: counts.total,
          indentCountsByStatus: counts 
        });
        
        // Also store in localStorage for quick access
        localStorage.setItem('indentCount', JSON.stringify(counts));
      },
