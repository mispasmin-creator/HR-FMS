...
const Sidebar = ({ onClose }) => {
  ...
  const [pendingAdminDepartmentCount, setPendingAdminDepartmentCount] =
    useState(0);
  const [pendingAccountDepartmentCount, setPendingAccountDepartmentCount] =
    useState(0);
  const [pendingStoreDepartmentCount, setPendingStoreDepartmentCount] =
    useState(0);
  ...
  useEffect(() => {
    const fetchPendingAfterJoiningCount = async () => {
      try {
        ...
        console.error(
          "Error fetching pending after joining count from Supabase:",
          error,
        );
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          result.data.forEach((item) => {
            ...
            const isComplete = ["complete", "completed", "fulfilled"].includes(
              statusStr,
            );
        ...
        console.error(
          "Error fetching pending indent count from Supabase:",
          error,
        );
      }
    };
  ...
  useEffect(() => {
    const fetchPendingSocialSiteCount = async () => {
      try {
        ...
        console.error(
          "Error fetching pending social site count from Supabase:",
          error,
        );
      }
    };
  ...
  useEffect(() => {
    const fetchPendingApprovalCount = async () => {
      try {
        ...
        console.log(
          "Raw data for pending approval count:",
          rawData.length,
          "rows",
        );
        ...
        dataRows.forEach((row, idx) => {
          ...
          if (idx < 5) {
            console.log(
              `Row ${idx + 7}: Column BG = "${columnBG}", Column BH = "${columnBH}"`,
            );
          }
          ...
          if (
            ...
            console.log(
              `Found pending approval: Row ${idx + 7}, BG = "${columnBG}", BH = "${columnBH}"`,
            );
          }
        });
  ...
  useEffect(() => {
    const fetchPendingAdminDepartmentCount = async () => {
      try {
        ...
        console.log(
          "Raw data for pending admin department count:",
          rawData.length,
          "rows",
        );
        ...
        dataRows.forEach((row, idx) => {
          ...
          if (idx < 5) {
            console.log(
              `Row ${idx + 7}: Column BS = "${columnBS}", Column BT = "${columnBT}"`,
            );
          }
          ...
          if (
            ...
            // Log the matching row
            console.log(
              `Found pending admin department: Row ${idx + 7}, BS = "${columnBS}", BT = "${columnBT}"`,
            );
          }
        });
  ...
  useEffect(() => {
    const fetchPendingAccountDepartmentCount = async () => {
      try {
        ...
        console.log(
          "Raw data for pending account department count:",
          rawData.length,
          "rows",
        );
        ...
        dataRows.forEach((row, idx) => {
          ...
          if (idx < 5) {
            console.log(
              `Row ${idx + 7}: Column BW = "${columnBW}", Column BX = "${columnBX}"`,
            );
          }
          ...
          if (
            ...
            // Log the matching row
            console.log(
              `Found pending account department: Row ${idx + 7}, BW = "${columnBW}", BX = "${columnBX}"`,
            );
          }
        });
        ...
        console.error(
          "Error fetching pending account department count:",
          error,
        );
        setPendingAccountDepartmentCount(0);
      }
  ...
  useEffect(() => {
    const fetchPendingStoreDepartmentCount = async () => {
      try {
        ...
        console.log(
          "Raw data for pending store department count:",
          rawData.length,
          "rows",
        );
        ...
        dataRows.forEach((row, idx) => {
          ...
          if (idx < 5) {
            console.log(
              `Row ${idx + 7}: Column CA = "${columnCA}", Column CB = "${columnCB}"`,
            );
          }
          ...
          if (
            ...
            // Log the matching row
            console.log(
              `Found pending store department: Row ${idx + 7}, CA = "${columnCA}", CB = "${columnCB}"`,
            );
          }
        });
      ...
      try {
        if (user?.Admin === "Yes") {
          ...
          if (result.success) {
            ...
            const pendingLeaving = result.pending.filter(
              (item) => item.planned_leaving_date && !item.actual_leaving_date,
            );
  ...
  useEffect(() => {
    const fetchPendingReportingManagerCount = async () => {
      try {
        ...
        console.log(
          "Raw data for pending reporting manager count:",
          rawData.length,
          "rows",
        );
        ...
        dataRows.forEach((row, idx) => {
          ...
          if (idx < 5) {
            console.log(
              `Row ${idx + 7}: Column BK = "${columnBK}", Column BL = "${columnBL}"`,
            );
          }
          ...
          if (
            ...
            // Log the matching row
            console.log(
              `Found pending reporting manager: Row ${idx + 7}, BK = "${columnBK}", BL = "${columnBL}"`,
            );
          }
        });
  ...
  useEffect(() => {
    const fetchSiesEmployeesCount = async () => {
      try {
        ...
        console.error(
          "Error fetching SIES employees count from Supabase:",
          error,
        );
  ...
  useEffect(() => {
    const fetchPendingITDepartmentCount = async () => {
      try {
        ...
        console.log(
          "Raw data for pending IT department count:",
          rawData.length,
          "rows",
        );
        ...
        dataRows.forEach((row, idx) => {
          ...
          if (idx < 5) {
            console.log(
              `Row ${idx + 7}: Column BO = "${columnBO}", Column BP = "${columnBP}"`,
            );
          }
          ...
          if (
            ...
            // Log the matching row
            console.log(
              `Found pending IT department: Row ${idx + 7}, BO = "${columnBO}", BP = "${columnBP}"`,
            );
          }
        });
    ...
    const fetchPendingLeaveCount = async () => {
      try {
        ...
        if (!Array.isArray(rawData) || rawData.length < 2) {
          console.log(
            "No data found or insufficient rows in Leave Management sheet",
          );
        ...
        console.log(
          "Raw data for pending leave count:",
          rawData.length,
          "rows",
        );
        ...
        dataRows.forEach((row, idx) => {
          ...
          if (idx < 5) {
            console.log(
              `Row ${idx + 2}: Employee: "${employeeName}", Status: "${status}"`,
            );
          }
          ...
          if (
            ...
            // Log the matching row
            console.log(
              `Found pending leave: Row ${idx + 2}, Employee: "${employeeName}", Status: "${status}"`,
            );
          }
        });
    ...
    const fetchEmployeeCount = async () => {
      try {
        ...
        dataRows.forEach((row, idx) => {
          ...
          const isActive =
            !status || status.toString().toLowerCase() !== "leaved";
          ...
          if (hasName && isActive) {
            ...
            if (idx < 5) {
              console.log(
                `Active employee ${idx + 1}: ${candidateName}, Status: ${status}`,
              );
            }
          }
        });
  ...
  useEffect(() => {
    const fetchUnprocessedPayrollCount = async () => {
      try {
        ...
        console.log(
          `Unprocessed payroll employees for ${currentMonth} ${currentYear}:`,
          pendingCount,
        );
    ...
    const fetchDeductionsCount = async () => {
      try {
        ...
        if (rawData.length > 1) {
          ...
          const advanceDeductionIndex = headers.findIndex(
            (header) =>
          ...
          dataRows.forEach((row, idx) => {
            ...
            const advanceAmount =
              advanceDeductionIndex >= 0 ? row[advanceDeductionIndex] : "";
            const hasAdvance =
              advanceAmount && advanceAmount.toString().trim() !== "";
...
`;
  ...
  if (
    ...
    !document.getElementById("scroll-prevention-styles")
  ) {
...
`;
  ...
  const adminMenuItems = [
...

    {
      ...
      badge:
        pendingReportingManagerCount > 0 ? pendingReportingManagerCount : null,
    },
    // {
    //   path: '/it-department',
    //   icon: UserMinus,
  ...
  const SidebarContent = ({ onClose, isCollapsed = false }) => {
    ...
    return (
      <div
        className={`flex flex-col h-full ${isCollapsed ? "w-16" : "w-64"} bg-gray-600 text-white`}
      >
        {/* Header */}
        ...
        <div className="flex-shrink-0 p-4 border-t border-white border-opacity-20">
          <div className="flex items-center mb-4 space-x-4">
            <div className="flex items-center space-x-2 cursor-pointer">
              <div className="flex items-center justify-center bg-indigo-100 rounded-full w-9 h-9">
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 hidden h-full lg:block">
        <SidebarContent />
      </div>
